...
impl TonemapperResources {
  pub fn new(dev: &D3D12Device, w: u32, h: u32, srcformat: DXGI_FORMAT, dstformat: DXGI_FORMAT) -> TonemapperResources {
    let h_tex =
      dev.create_committed_resource(
          ...
          D3D12_RESOURCE_STATE_COMMON, None).expect("Cannot create horizontal temp texture");

    let tex_desc = resource_desc_tex2d_nomip(w as u64, h as u32, dstformat, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    trace!("Create intermediate texture");
    // intermediate texture for copying into back-buffer
    let im_tex = dev.create_committed_resource(&heap_properties_default(), D3D12_HEAP_FLAG_NONE, 
                              &tex_desc, D3D12_RESOURCE_STATE_COMMON, None).expect("Cannot create dest temp texture");
  ...
  pub fn resize(&mut self, core: &DXCore, w: u32, h: u32, format: DXGI_FORMAT) -> HResult<()> {
    trace!("Create horizontal intermediate texture");
    self.h_tex =
      try!(core.dev.create_committed_resource(
          &heap_properties_default(), D3D12_HEAP_FLAG_NONE, 
          &resource_desc_tex2d_nomip(w as u64, h as u32, 
              self.srcformat, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS), 
          D3D12_RESOURCE_STATE_COMMON, None));

    let tex_desc = resource_desc_tex2d_nomip(w as u64, h as u32, format, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    trace!("Create intermediate texture");
    // intermediate texture for copying into back-buffer
    self.im_tex = try!(core.dev.create_committed_resource(&heap_properties_default(), D3D12_HEAP_FLAG_NONE, 
...
impl Tonemapper {
  ...
  // t_fence will hold temporary resources
  pub fn tonemap(&self, core: &DXCore, res: &TonemapperResources, src: &D3D12Resource, dst: &D3D12Resource, t_fence: &mut Fence) -> HResult<()> {
    ...
    core.dev.create_shader_resource_view(Some(&res.h_tex), Some(&h_desc), self.dheap.cpu_handle(1));

    let uav_desc = uav_tex2d_desc(dstdesc.Format, 0, 0);
    core.dev.create_unordered_access_view(Some(&res.im_tex), None, Some(&uav_desc), self.dheap.cpu_handle(2));
    ...
    // t_fence will keep im_tex and h_tex alive 
    //t_fence.hold(im_tex).hold(h_tex).signal(&core.graphics_queue, core.next_fence_value());
