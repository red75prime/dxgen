...
pub fn create_dxgi_factory2<T: HasIID>() -> HResult<T> {
  ...
  let hr=unsafe {CreateDXGIFactory2(if cfg!(debug_assertions) {DXGI_CREATE_FACTORY_DEBUG} else {0}, T::iid(), &mut p_fac as *mut *mut _ as *mut *mut _) }; 
  if hr==0 {
    Ok(T::new(p_fac))
  } else {
    Err(hr)
...
extern "system" {
  fn D3DCompileFromFile(file_name: LPCWSTR, defines: *const D3D_SHADER_MACRO, include: *mut ID3DInclude, 
      entry: LPCSTR, target: LPCSTR, flags1: UINT, flags2: UINT, code: *mut *mut ID3D10Blob, errs: *mut *mut ID3D10Blob) -> HRESULT;

  fn D3DCompile2(pSrcData: LPCVOID, SrcDataSize: SIZE_T, pSourceName: LPCSTR, pDefines: *const D3D_SHADER_MACRO,
      pInclude: *mut ID3DInclude, pEntrypoint: LPCSTR, pTarget: LPCSTR, Flags1: UINT, Flags2: UINT, 
...
pub fn d3d_compile_from_str(shader_text: &str, entry: &str, target: &str, flags: u32) -> Result<Vec<u8>, String> {
  ...
  let hr = unsafe{ D3DCompile2(shader_text.as_ptr() as *const _, shader_text.len() as SIZE_T, ptr::null(), ptr::null(), ptr::null_mut(), cstr_entry.as_ptr(),
            cstr_target.as_ptr(), flags, 0, 0, ptr::null(), 0, &mut code_blob, &mut error_blob)};
  if SUCCEEDED(hr) {
...
pub fn d3d_get_root_signature_from_str(shader_text: &str, dev: &D3D12Device) -> Result<D3D12RootSignature,HRESULT> {
  let mut root_sign_blob: *mut ID3DBlob = ptr::null_mut();
  let hr = unsafe{ D3DGetBlobPart(shader_text.as_ptr() as *mut _ , shader_text.len() as SIZE_T, D3D_BLOB_ROOT_SIGNATURE, 0, &mut root_sign_blob) };
  if SUCCEEDED(hr) {
