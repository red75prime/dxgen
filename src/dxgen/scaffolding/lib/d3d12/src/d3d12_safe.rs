...
impl D3D12Device {
  ...
  pub fn check_feature_support_virtual_address(&self, feature_support_data: &mut D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT) -> HResult<HRESULT> {
    ...
    let hr=unsafe { (*(self.0 as *mut ID3D12Device)).CheckFeatureSupport(D3D12_FEATURE_GPU_VIRTUAL_ADDRESS_SUPPORT, feature_support_data as *mut _ as *mut _, mem::size_of_val(feature_support_data) as UINT) };
    hr2ret(hr,hr)
  }
