...
impl DXGIFactoryMedia {
  ...
  pub fn create_swap_chain_for_composition_surface_handle<T: HasIID>(&self, device: &T, hSurface: HANDLE, desc: &DXGI_SWAP_CHAIN_DESC1, restrict_to_output: Option<&DXGIOutput>) -> HResult<DXGISwapChain1> {
    let mut lv1: *mut IDXGISwapChain1 = ptr::null_mut();
    let _hr=unsafe { (*(self.0 as *mut IDXGIFactoryMedia)).CreateSwapChainForCompositionSurfaceHandle(device.iptr() as *mut _ as *mut _ , hSurface, desc, restrict_to_output.map(|i|i.iptr()).unwrap_or(ptr::null_mut()) as *mut _ as *mut _, &mut lv1 as *mut *mut _) };
    hr2ret(_hr,DXGISwapChain1::new(lv1 as *mut _))
  }
