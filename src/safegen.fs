...
let emptyAnnotationsGen (types:Map<string,CTypeDesc*CodeLocation>,enums:Map<string,CTypeDesc*CodeLocation>,structs:Map<string,CTypeDesc*CodeLocation>,
                            funcs:Map<string,CTypeDesc*CodeLocation>, iids:Map<string,CodeLocation*IID>, defines:Map<string, MacroConst*string*CodeLocation>, typedefs) : string=
      ...
      |ANone ->
        ...
        |[] ->
          ...
          |TypedefRef "LPCWSTR" |Ptr(Const(TypedefRef "WCHAR")) ->
            ...
            let gt = newGenType("AsRef<str>")
            addLocalVar locVar false (RType "Vec<u16>") (fun m -> "str_to_vec_u16("+safeParmName+".as_ref().into())")
            addSafeParm safeParmName (RGeneric(gt,"AsRef<str>"))
...
let safeInterfaceGen (header:string) (uses_CanBeFreakingNull: string seq) allInterfaces noEnumConversion 
                     (types:Map<string,CTypeDesc*CodeLocation>, enums:Map<string,CTypeDesc*CodeLocation>, 
                            structs:Map<string,CTypeDesc*CodeLocation>, funcs:Map<string,CTypeDesc*CodeLocation>, 
                            iids:Map<string,CodeLocation*IID>, defines:Map<string, MacroConst*string*CodeLocation>,
                            typedefs) 
